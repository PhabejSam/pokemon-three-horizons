...
{
    struct THPartnerOptions options = {.nature = NATURE_ADAMANT, .shiny = TRUE, .abilityNum = 2};
    for (u32 i = 0; i < NUM_STATS; i++) options.ivs[i] = 31;
    options.evs[STAT_HP] = 6;
    options.evs[STAT_ATK] = 252;
    options.evs[STAT_SPEED] = 252;
    TH_CreateConfiguredPartner(mon, SPECIES_MAGIKARP, 5, &options);
}

void TH_ApplyWildHiddenAbility(struct Pokemon *mon, u32 roll)
{
    u16 species = GetMonData(mon, MON_DATA_SPECIES);
    if (roll < 10 && species != SPECIES_NONE && GetSpeciesAbility(species, 2) != ABILITY_NONE)
    {
        u8 slot = 2;
